...
if aws iam get-role --role-name delegate-admin-OSDCCSAdmin ;then
   aws iam delete-role --role-name delegate-admin-OSDCCSAdmin || echo "Role was unable to be deleted"
   aws iam delete-policy --policy-arn arn:aws:iam::aws:policy/AdministratorAccess|| echo "Policy was unable to be deleted"
else 
   aws iam create-role --role-name delegate-admin-OSDCCSAdmin --assume-role-policy-document file://OSDCCSAdmin_IAM_role.json || echo "Role not created"
   aws iam attach-role-policy --role-name delegate-admin-OSDCCSAdmin --policy-arn arn:aws:iam::aws:policy/AdministratorAccess || echo "Policy not attached"
fi
