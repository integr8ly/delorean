...
create_cluster() {
    ...
    wait_for "ocm get subs --parameter search=\"cluster_id = '${cluster_id}'\" | jq -r .items[0].metrics[0].health_state | grep -q -E ^healthy" "cluster to be healthy" "${TIMEOUT_CLUSTER_HEALTH_CHECK}m" "30" \
...
install_rhods_addon() {
    ...
    echo "${addon_payload}" | ocm post "/api/clusters_mgmt/v1/clusters/${cluster_id}/addons"
    wait_for "ocm list addons --cluster=${cluster_id} |grep managed-odh |grep ready" "RHODS addon to be installed" "35m" "300"
}
