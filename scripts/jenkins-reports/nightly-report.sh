...
declare -a DEFAULT_JOBS=("rhmi-install-addon-flow" "rhmi-install-master" "rhmi-openshift-upgrade" "rhmi-upgrade")
declare -a jobs
...
do
  ...
  do
    ...
    errorCheck=$(grep "$ERROR_STRING" build.json)
    ...
    buildTimestamp=$(jq -r .timestamp build.json)/1000
    
    if [ $PERIOD_END_TIMESTAMP -gt "$buildTimestamp" ] ; then
      break
    fi

    causedByDesc=$(jq -r .actions[].causes[0].shortDescription build.json | grep -vwE "null")
    
    if [[ $causedByDesc == *"timer"* ]] ; then
      buildUrl=$(jq -r .url build.json)
      getDateString
      buildResult=$(jq -r .result build.json)
      echo -e "\n\t#$buildNumber\t$buildDate\tResult: $buildResult\tUrl: $buildUrl" >> report.txt

      description=$(jq -r .description build.json)
